...
function selectBestFile(val, route, records) {
	...
	function upChain(thisVal, keyPath, valuesPath) {
		...
		const thisIsRouteClass = isRouteClass(thisVal);
		...
		for (const ref of nodeRefs) {
			if (typeof ref === 'string') {
				...
				const thisKey = ref.key;
				if (thisIsRouteClass && isSymbol(thisKey)) continue;
				upChain(ref.parent, [thisKey, ...keyPath], new Set(valuesPath).add(thisVal));
			}
		}
	}
