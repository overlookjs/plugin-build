 ...
 * i.e. Whether it can be (in order of preference):
...
function getLocation(val, records) {
	...
	for (let thisLocation of locations) {
		if (isString(thisLocation)) {
			...
			const parentInRoute = parentLocation.inRoute;
			if (parentInRoute) {
				if (!inRoute) {
	...
	if (location) {
		if (valIsRoute) location.inRoute = val;
	} else {
		let path, key;
		if (valIsRoute) {
...
function getPathLocation(path) {
	...
	return {path, parent: null, key: null, packageDepth, pathDepth, keyDepth: 0, inRoute: undefined};
}
