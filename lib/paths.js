...
function getLocation(val, records) {
	...
	if (record.ref != null) return getPathLocation(record.ref);
	...
	for (const ref of record.refs) {
		...
		if (isString(ref)) {
			...
			const {parent} = ref;
			if (!parent) continue;
			const parentProps = getLocation(parent, records);
			thisLocation = {...parentProps, keyPath: [...parentProps.keyPath, ref.key]};
		}
...
function getPathLocation(path) {
	...
	return {path, packageDepth, pathDepth, keyPath: []};
}
