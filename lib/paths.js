...
// Imports
const {createLocation} = require('./trace.js'),
...
function getLocation(val, records) {
	...
	// Set location to null to prevent infinite loops on circular references
	...
	for (let thisLocation of record.locations) {
		if (isString(thisLocation)) {
			...
			thisLocation = createLocation({
				...parentLocation,
				keyPath: [...parentLocation.keyPath, thisLocation.key]
			});
		}
...
function getPathLocation(path) {
	...
	return createLocation({path, packageDepth, pathDepth, keyPath: []});
}
