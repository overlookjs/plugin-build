...
const {join: pathJoin, sep: pathSep} = require('path'),
	...
	{isString, isFunction} = require('is-it-type');
...
function isItemToDebug(val) { // eeslint-disable-line no-unused-vars
	return isFunction(val) && val.toString() === 'function(x) { return x; }';
}
...
function locate(val, records) {
	...
	const isDebugFn = false; // isItemToDebug(val);
	const isDebugProto = false; // isItemToDebug(val.constructor);
	// eslint-disable-next-line no-nested-ternary
	const debugPrefix = isDebugFn ? 'fn' : isDebugProto ? 'prototype' : null;
	const debug = debugPrefix
		// eslint-disable-next-line no-console
		? (name, ...vals) => console.log(`${debugPrefix} ${name}:`, ...vals)
		: () => {};
	...
	for (let thisLocation of locations) {
		if (isString(thisLocation)) {
			...
			debug('thisLocation', thisLocation);
			...
			if (parentInDynamic === true) continue;
			if (parentInDynamic) {
				if (!inDynamic) {
	...
	debug('location before', {location, inDynamic, isDynamic, hasCircular});
	...
	debug('location after', location, hasCircular);

	// if (isDebugFn) process.exit(); // eslint-disable-line no-process-exit
...
function getPathLocation(path) {
	...
	const packageParts = path.split(NODE_MODULES_PATH_SEGMENT);
	if (packageParts.length > 1) {
		...
		path = path.slice(packageParts[0].length + NODE_MODULES_PATH_SEGMENT.length);
