...
module.exports = {
	 ...
	 * Need to serialize each function separately so they get references.
	 *
	 * What need to do is:
	 * - parse the entire class with babel
	 * - get source for each method from babel (using start/end chars)
	 * - match up the methods to object's properties by comparing source
	 *   (NB static methods don't include `static ` in `fn.toString()`)
	 *   (NB any methods with same source as each other can be differentiated by order)
	 * - define all matching methods within class def + set their location as class as parent
	 *   (NB location could already be recorded and wrong if also in another file -
	 *   need to fix this earlier on when locating -
	 *   if this is a method (i.e. no `function` in `.toString()` output), that takes priority)
	 * - call `this.serializeValue()` on all other properties and include as dependencies
	 *   with placeholders
	 */
	...
	serializeClassProps(props, staticJs, placeholderJs, methodJss, dependencies, replaceVar) {
		...
		for (const {key, val} of props) {
			...
			if (isFunction(val)) {
				...
				propJs = this.serializeValue(val);
