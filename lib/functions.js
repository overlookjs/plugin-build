...
module.exports = {
	...
	serializeClass(klass, superClass, classJs, record) {
		...
		const protoRes = getObjectProps(proto, key => key === 'constructor' && proto.constructor === klass);
		dependencies.push(protoRes.dependencies);
		// TODO Should reference prototype var, not `.prototype` directly
		const protoPropsJs = this.serializeClassProps(
			protoRes.props, '', `${placeholderJs}.prototype`, methodJss, dependencies, replaceVar
		);
		...
		dependencies.push(staticRes.dependencies);
		...
		const js = 'class ' // eslint-disable-line prefer-template
			+ klass.name ? `${klass.name} ` : '' // eslint-disable-line prefer-template
			+ extendsJs
			+ '{\n'
