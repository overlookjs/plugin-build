...
class RouteClassSerializer {
	...
	getModulePath(obj, fallback) {
		return getModulePath(obj, fallback, this.destPath);
	}

	getPath(path) {
		return getPath(path, this.destPath);
	}
}
...
function getModulePath(obj, fallback, destPath) {
	const module = obj[MODULE];
	return module ? getPath(module.filename, destPath) : fallback;
}

function getPath(path, destPath) {
	path = pathRelative(destPath, path).slice(1);
