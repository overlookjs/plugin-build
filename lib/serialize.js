...
class Serializer {
	...
	serializeFunction(fn, varName) {
		const record = this.initVar(fn, fn.name || varName);
		const fnName = getVarPlaceholder(record);
		if (!record.js) {
			const fnStr = getFunctionCode(fn);
			let js;
			if (fnStr.startsWith('class ')) {
				js = this.serializeClass(fn, fnStr);
			} else {
				js = this.serializeFunctionStandard(fn, fnStr, fnName);
			}
			record.js = js;
		}
		return fnName;
	...
	serializeFunctionStandard(fn, js, fnName) {
		const props = this.getObjectProps(
			fn, key => ['length', 'name', 'arguments', 'caller', 'prototype'].includes(key)
		);
		js = wrapWithProps(js, props);

		const protoProps = this.getObjectProps(fn.prototype, key => key === 'constructor');
		if (protoProps.length > 0) {
			// Has an active prototype - force it to be a var
			this.initVar(fn);
	...
	serializeClass(fn, fnStr) { // eslint-disable-line class-methods-use-this
		return fnStr;
	}
	...
	serializeClassInstance(val, proto) {
		...
		if (!protoRecord.js) {
			const fnName = this.serializeFunction(ctor);
			protoRecord.js = `${fnName}.prototype`;
		}
		// `.initVar` is here rather than above to ensure function referenced before prototype
		this.initVar(proto, `${ctor.name}Prototype`);

		return this.wrapWithProps(
			val, `Object.create(${getVarPlaceholder(protoRecord)})`,
