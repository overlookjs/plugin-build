...
const {trace, createRecord} = require('./trace.js'),
	{resolveFilePath} = require('./paths.js'),
	{
...
class Serializer {
	...
	serializeRouteBaseClass(Class) {
		this.getLocalVarName(Class, 'Route', true);
		return getVarPlaceholder(this.getRecord(Class));
	}

	serializePlugin(Class, plugin, previousClassName) {
		this.getLocalVarName(plugin, getPluginVarName(plugin.name), true);
		this.getLocalVarName(Class, Class.name, true);

		const record = this.getRecord(Class);
		if (!record.js) {
			const pluginVarName = getVarPlaceholder(this.getRecord(plugin));
			record.js = `${previousClassName}.extend(${pluginVarName})`;
		}
	...
	serializeSubclass(Class, extendsName) {
		...
		// Get name
		this.getLocalVarName(Class, statedName || Class.name, true);
		const record = this.getRecord(Class);
		const name = getVarPlaceholder(record);
