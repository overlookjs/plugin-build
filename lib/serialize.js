...
const valuesMethods = require('./values.js'),
	functionsMethods = require('./functions.js'),
...
class Serializer {
	...
	serializeRoute(route) {
		...
		if (path !== null && path !== '?') {
			...
			if (path === '?') {
				...
				res = this.serializeClassInstanceWithoutProps(Object.getPrototypeOf(route));
			} else {
				// Route exists as property of file
				res = this.serializeParentAccess(record);
			}

			routeJs = `${res.js};\nmodule.exports = ${varName}`;
			wrapJs = js => `'use strict';\n\n${js}\n`;
			this.addLocalDependencies(res.dependencies, localVars);
		}
	...
	addLocalVar(val, localVars) {
		...
		if (localVar) {
			...
			// TODO Find better way to identify multi-statement vars
			localVar = {isActive: record.js.includes('\n')};
...
// Merge in methods from `.values.js` and `functions.js`
Object.assign(Serializer.prototype, valuesMethods, functionsMethods);
