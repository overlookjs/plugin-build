...
class Serializer {
	...
	serializeRoot(root) {
		...
		// If another file already occupying this position, flag that need a nested folder in build
		let {srcPath} = this;
		...
		if (rootRecord.path !== entryPath) {
			if (this.existingPaths.has(entryPath)) {
				srcPath = `${srcPath}..${pathSep}`;
				entryPath = `${srcPath}index.js`;
				this.nestBuildDir = true;
			}
	...
	serializeSharedFile(val, record) {
		const name = getVarName(val, record);
	...
	resolvePlaceholders(mainJs, localVars) {
		...
		for (const [val, localVar] of localVars) {
			...
			if (localVar.isActive) {
				let varName = localVar.name || getVarName(val, record);
	...
	resolveRoutePath(route) {
		...
		if (route[SRC_DIR]) {
			...
			if (parent) {
				...
				pathWithoutExt = `${parentDirPath}${pathSep}${getVarName(route, record) || 'anon'}`;
			} else {
 ...
 * @returns {boolean} .nestBuildDir - `true` if build dir needs a nested folder
 */
function serialize(root, srcPath, nodeModulesPath, records) {
	...
	return {
		...
		nestBuildDir: serializer.nestBuildDir
	};
}
