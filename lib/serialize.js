...
class Serializer {
	...
	serializeRoot(root) {
		...
		if (rootRecord.path !== entryPath) {
			...
			const js = "'use strict';\n\n"
				+ `module.exports = ${createRequirePlaceholder(rootRecord.id)};\n`;
		...
		if (sharedFiles.length > 0) {
			...
			for (const sharedFile of sharedFiles) {
				sharedFile.path = this.getUniquePath(`${sharedDirPath}${sharedFile.name}`);
			}
		}
	...
	serializeRoute(route) {
		...
		record.js = createRequirePlaceholder(id);
	...
	serializeSharedFile(val, record) {
		...
		const {js: baseJs, dependencies} = this.serializeObjectLike(val);
		...
		let js = this.resolvePlaceholders(
			`module.exports = ${createValueIdPlaceholder(record.id)}`, localVars
		);
		js = `'use strict';\n\n${js};\n`;

		this.sharedFiles.push({name: varName, js, localVars});

		record.js = createRequirePlaceholder(record.id);
	}
	...
	serializeNonPrimitive(val, record) {
		...
		if (path != null) { // TODO Make path always `null`, never undefined
			...
			if (pathIsAbsolute(path)) {
				js = createRequirePlaceholder(record.id);
...
function createRequirePlaceholder(id) {
