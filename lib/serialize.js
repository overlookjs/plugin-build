...
class Serializer {
	...
	serializeFunction(fn, varName) {
		...
		if (!record.js) {
			...
			if (fnStr.startsWith('class ')) {
				...
				js = this.serializeFunctionStandard(fn, fnStr);
			}
			record.js = js;
		}
		return getVarPlaceholder(record);
	...
	serializeFunctionStandard(fn, js) {
		const props = this.getObjectProps(
			fn, key => ['length', 'name', 'arguments', 'caller', 'prototype'].includes(key)
		);

		const proto = fn.prototype;
		const protoProps = this.getObjectProps(
			proto, key => key === 'constructor'
		);

		if (protoProps.length > 0) {
			props.push({
				nameJs: serializePropertyKey('prototype'),
				accessJs: serializePropertyAccess('prototype'),
				valJs: serializeObjectFromProps(protoProps)
			});
		}
	...
	serializeClassInstance(val, proto) {
		...
		const protoRecord = this.initVar(proto, `${ctor.name}Prototype`);
		if (!protoRecord.js) {
