...
class Serializer {
	...
	serializeFunctionStandard(fn, js, varName) {
		...
		const protoProps = this.getObjectProps(proto, key => key === 'constructor'),
			hasPrototype = protoProps.length > 0;
		if (hasPrototype && computeJs) {
			js += `;\n${wrapWithProps(`${fnName}.prototype`, protoProps)}`;
		}

		if (computeJs) record.js = wrapWithProps(js, props);

		if (fn.name) varName = fn.name;
		this.initVar(fn, varName, hasPrototype); // Force this to be a var if has prototype
