...
class Serializer {
	...
	serializeRouteInstance(route, destPath) {
		...
		const instanceRecord = this.getVarRecord(route, instanceName, srcPath);
		instanceRecord.js = `Object.create(${className}.prototype)`;
	...
	serializeObjectType(val, ref) {
		...
		const varRecord = this.getVarRecord(val, varName, this.srcPath, ref);
	...
	serializeBuiltInObject(val, type, ref) {
		if (type === 'RegExp') return this.serializeRegex(val, ref);
		if (type === 'Date') return this.serializeDate(val, ref);
		if (Array.isArray(val)) return this.serializeArray(val, ref);
		if (type === 'Set' || type === 'Map') return this.serializeSetOrMap(val, type, ref);
		if (type === 'Uint8Array') return this.serializeBuffer(val, ref);
		throw new Error(`Cannot serialize ${val} (type ${type})`);
	}

	serializeRegex(regex, ref) {
		const js = `/${regex.source}/${regex.flags}`;
		return this.wrapWithProps(regex, js, ref, key => key === 'lastIndex');
	}

	serializeDate(date, ref) {
		const js = `new Date(${date.getTime()})`;
		return this.wrapWithProps(date, js, ref);
	}
	...
	serializeSetOrMap(setOrMap, type, ref) {
		const js = `new ${type}([${Array.from(setOrMap).map((val, index) => this.serializeValue(val, [...ref, index])).join(', ')}])`;
		return this.wrapWithProps(setOrMap, js, ref);
	}

	serializeBuffer(buf, ref) {
		const js = `Buffer.from(${serializeString(buf.toString('base64'))}, ${serializeString('base64')})`;
		return this.wrapWithProps(buf, js, ref, key => key === '0' || key.match(/^[1-9]\d*$/));
	}

	wrapWithProps(obj, js, ref, shouldSkipProp) {
		const extendJs = this.serializeObject(obj, ref, shouldSkipProp);
		if (extendJs === '{}') return js;
		return `Object.assign(${js}, ${extendJs})`;
	}

	serializeObject(obj, ref, shouldSkipProp) {
		const props = this.getObjectProps(obj, ref, shouldSkipProp);
		return `{${props.map(prop => `${prop.nameJs}: ${prop.valJs}`).join(', ')}}`;
	}

	getObjectProps(obj, ref, shouldSkipProp) {
		const props = [];

		for (const propName of Object.getOwnPropertyNames(obj)) {
			if (shouldSkipProp && shouldSkipProp(propName)) continue;
	...
	serializeSymbol(symbol) {
		...
		const varRecord = this.getVarRecord(symbol, symbolName);
	...
	serializePluginSymbolStore(pluginName) {
		...
		const varRecord = this.getVarRecord(store, varName);
	...
	serializeSymbolStore() {
		// Create var record for global symbol store
		const varRecord = this.getVarRecord(symbolStore, 'symbolStore', '@overlook/symbol-store');
	...
	getVarRecord(val, varName, path, ref) {
		...
		// Add reference
		if (path) varRecord.refs.push({path, ref: ref || []});
