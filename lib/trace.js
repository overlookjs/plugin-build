...
const {builtinModules} = require('module'),
	...
	// TODO Export these Symbols from `class-extension` explicitly
	{CACHE, EXTENSIONS, NAMED_EXTENSIONS} = require('class-extension/lib/symbols.js'),
	{isFunction, isSymbol} = require('is-it-type'),
	hasOwnProperty = require('has-own-prop'),
	{forIn} = require('lodash');

// Imports
const {isPrimitive} = require('./primitives.js');
 ...
 * `nodeModulesPath` is path to `node_modules` directory.
 * Any module not within these paths is ignored.
 * Each path must end with path separator (i.e. '/' on posix)
 * @param {string} srcPath - Source directory path
 * @param {string} nodeModulesPath - `node_modules` directory path
 * @returns {Map} - Map of records for all objects
 */
function trace(srcPath, nodeModulesPath) {
	// Init records map
	...
	traceModules(srcPath, nodeModulesPath, records);
...
function traceModules(srcPath, nodeModulesPath, records) {
	forIn(
		require.cache,
		mod => traceModule(mod, srcPath, nodeModulesPath, records)
	);
}

function traceModule(mod, srcPath, nodeModulesPath, records) {
	...
	// Do not create location for `@overlook/symbol-store` package
	// - symbols should be imported from plugins.
	const path = mod.filename;
	if (
		(path.startsWith(srcPath) || path.startsWith(nodeModulesPath))
		&& !path.startsWith(`${nodeModulesPath}@overlook${pathSep}symbol-store${pathSep}`)
	) locations.push(path);
...
function traceObject(obj, records, shouldSkipKey) {
	...
	if (symbols.length > 0) {
		...
		// Add reference to symbol keys array on record for object
		...
		for (const symbol of symbols) {
			...
			if (symbol === CACHE || symbol === EXTENSIONS || symbol === NAMED_EXTENSIONS) continue;
