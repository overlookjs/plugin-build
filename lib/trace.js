...
function traceGlobal(records) {
	...
	globalRecord.location = createLocation({
		path: undefined,
		keyPath: [],
		packageDepth: 0,
		pathDepth: 0
	});
	globalRecord.locations.length = 0;
	...
	for (const key of Object.getOwnPropertyNames(global)) {
		...
		record.location = createLocation({
			path: undefined,
			keyPath: [],
			packageDepth: 0,
			pathDepth: 0
...
function traceBuiltInModules(records) {
	for (const name of builtinModules) {
		...
		} else if (record.location !== undefined) {
...
function traceValue(val, parent, key, records) {
	...
	if (parent) record.locations.push(createLocation({parent, key}));
