...
class Serializer {
	...
	serializeRoute(route) {
		...
		record.js = createRequirePlacholder(record.id);
		...
		if (path !== null) {
			...
			initialJs = `const ${varName} = module.exports;\n\n`;
			wrapJs = js => (
			...
			const {js: routeJs, dependencies: routeDependencies} = this.serializeParentAccess(record);
			initialJs = `const ${varName} = ${routeJs};\n`
				+ `module.exports = ${varName};\n\n`;
			wrapJs = js => `'use strict';\n\n${js}`;
			initialDependencies = routeDependencies;
		}

		const {props, dependencies} = getObjectProps(route);
		if (initialDependencies) dependencies.unshift(...initialDependencies);
		...
		localVars.set(route, {isActive: true, name: varName});
		...
		this.routeFiles.push({path, js, localVars});
	}
...
function serialize(root, srcPaths, records) {
	...
	console.log('existingPaths:', serializer.existingPaths);
	return {
